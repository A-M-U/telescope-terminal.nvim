...
local settings = require('terminal.settings').get()

local M = {}

M.stuff = function(opts)
  ...
  opts = opts or {}
   ...
   for _, bufnr in ipairs(bufnrs) do
      ...
      -- print(info.variables.toggle_number)
      local element = {
         bufnr = info.bufnr,
   ...
   table.insert(buffers, {bufnr = 42, name = settings.default})
   pickers.new(opts, {
    ...
    attach_mappings = function(prompt_bufnr, map)
      actions.select_default:replace(function()
        ...
        -- vim.api.nvim_command("ToggleTerm ".. selection[1])
        -- actions.close(prompt_bufnr)
        -- local selection = action_state.get_selected_entry()
        -- -- print(vim.inspect(selection))
